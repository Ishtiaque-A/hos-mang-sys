...
const HolidayGroup = () => {
    ...
    useEffect(() => {
        axios.get(`/holiday-group`).then(res => {
            if (res.data.status === 200) {
                    ...
                    <div className="col-md-9 mt-2">
                        <div className="card">
                            <div className="card-header">
                                <h1 className="card-title">Holiday Group
                                <Link to={'/add-holiday-group'} className="btn btn-primary btn-sm float-end"> Add  </Link>
                                </h1>
                            </div>
                            <div className="card-body">
                                    <MaterialTable
                                        columns={[
                                            { title: "Holiday Group", field: "name", render: rowData => <td>{rowData.name}</td> },
                                            {
                                                title: "Action",
                                                field: "action",
                                                render: rowData => <td> <Link to={`/edit-holiday-group/${rowData.id}`} class="btn btn-info btn-sm action-btn"><i class="fas fa-edit"></i></Link>
