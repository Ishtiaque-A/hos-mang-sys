  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`recommendation/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
    ...
    <div className="m-2">
      <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
        <div className="d-flex justify-content-between align-items-center">
          <h6 className="mt-1 mx-2">Recommendation</h6>
...
const AddSpecimen = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post("recommendation", { title });
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForAdd(false)}>
        <NewModal.Title>Add Recommendation</NewModal.Title>
      </NewModal.Header>
...
const EditSpecimen = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const title = e.target.title.value;
      const res = await axios.put(`recommendation/${data?.id}`, {
        title,
      });
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForEdit(false)}>
        <NewModal.Title>Update Recommendation</NewModal.Title>
      </NewModal.Header>
      <form onSubmit={handleSubmit} className="mt-3">
        <NewModal.Body>
          <div className="form-group">
