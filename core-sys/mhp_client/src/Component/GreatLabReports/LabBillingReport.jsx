...
export default function LabBillingReport() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .post(`great-lab-billing-report`, filter)
      .then((res) => {
    ...
    axios.get(`/doctors`).then(async (res) => {
  ...
  const handleFilter = () => {
    ...
    axios
      .post(`great-lab-billing-report`, modifiedData)
      .then((res) => {
        setData(res?.data?.invoice || []);
        setAllData(res?.data || {});
      })
          ...
          <div className={isSuperAdmin ? "col-5 d-flex" : "col-6 d-flex"}>
            ...
            >
              <ReactSearchAutocomplete
                ...
                onSelect={(item) => {
                  setFilter({ ...filter, patient_id: item?.id });
                }}
                maxResults={3}
                fuseOptions={{
                  keys: [
            ...
            >
              <ReactSearchAutocomplete
                ...
                inputSearchString={searchDoctorName || ""}
                onSearch={(value) => setSearchDoctorName(value)}
                searchInputProps={{
                  className: "form-control",
                  placeholder: "Search",
                }}
                ...
                resultStringKeyName="fullName"
                onSelect={(item) => {
                  setFilter({ ...filter, doctor_id: item?.id });
                }}
                maxResults={3}
                fuseOptions={{
                  keys: ["dr_identity_no", "fullName"],
                }} // Search in the description text as well
                styling={{
                  borderRadius: "10px !important",
                  // zIndex: modalIsOpen || isOpenForPaymentModal ? "auto" : "20",
                  ...
                  <>
                    <tr key={index}>
                      ...
                      <td>
                        {item?.doctor ? (
                          <>
                            {item?.doctor?.title?.title_name}{" "}
                            {item?.doctor?.fullName}{" "}
                            {item?.doctor?.specialist?.specialists_name
                              ? `(${item?.doctor?.specialist?.specialists_name})`
                              : ""}
                          </>
                        ) : (
