...
export default function LabEmployeeShiftReport() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .post(`great-lab-billing-report-user`, filter)
      .then((res) => {
    ...
    axios.get(`/lab-users`).then(async (res) => {
      setLabUsers(res.data.users);
    });
    axios.get(`/shift`).then(async (res) => {
      const updatedShift = res?.data?.map((shift) => ({
        ...shift,
        value: shift.id,
        label: shift.name,
      }));
  ...
  const handleFilter = () => {
    ...
    axios
      .post(`great-lab-billing-report-user`, modifiedData)
      .then((res) => {
        setData(res?.data?.invoice || []);
        setAllData(res?.data || {});
      })
  ...
  const handleFilterShift = (data) => {
    if (data) {
      setFilter({ ...filter, shift: data, shift_id: data?.id });
    } else {
      setFilter({ ...filter, shift_id: null, shift: null });
    }
  };
    ...
    <div className="ms-2 mt-2">
      <div className="shadow-sm p-2 mb-3 bg-body lab-points-plan rounded mt-1">
        <div className="custom-card p-2">
          <div className="row">
            <div className={isSuperAdmin ? "col-5 d-flex" : "col-6 d-flex"}>
              ...
              >
                <ReactSearchAutocomplete
                  ...
                  onSelect={(item) => {
                    setFilter({ ...filter, user_id: item?.id });
                  }}
                  maxResults={3}
                  fuseOptions={{
                    keys: ["name"],
              ...
              <div
                style={{ width: isSuperAdmin ? "200px" : "200px" }}
                className="lab-agent-search ms-2 "
              >
                <SimpleSelect
                  value={filter?.shift || null}
            ...
            <div className={isSuperAdmin ? "col-7" : "col-6"}>
              <div className="row">
          ...
          {loading ? (
            ...
            <table>
              <tr className="report-header-bg">
                ...
                <td className="text-end">Received Amount</td>
                ...
                return (
                  <>
                    <tr key={index}>
                      ...
                      <td className="text-right">{item?.totalDiscount || 0}</td>
                      <td className="text-right">{item?.paidAmount}</td>
              ...
              <tr>
                ...
                <td className="text-right fw-bold">{allData?.paid}</td>
                <td className="text-right fw-bold">{allData?.refund}</td>
                <td className="text-right fw-bold">{allData?.due}</td>
              </tr>
              ...
              {allData?.dueInvoice?.map((inv, index) => {
                let totalDiscount =
                  parseFloat(inv?.invoice_only?.discount || 0) +
                  parseFloat(inv?.invoice_only?.specialDiscount || 0);
                return (
                  <tr key={index}>
                    <td>
                      {moment(inv?.invoice_only?.created_at)?.format(
                        "DD/MM/YYYY hh:mm A"
                      )}
                    </td>
                    <td>{inv?.invoice_only?.invoiceNo}</td>
                    <td>{inv?.invoice_only?.patient?.fullName}</td>
                    {isSuperAdmin && (
                      <td>{inv?.invoice_only?.saas_branch_name}</td>
                    )}
                    <td>{inv?.shift?.name}</td>
                    <td>{inv?.invoice_only?.patient?.created_by}</td>
                    <td>{inv?.invoice_only?.paymentMethod}</td>
                    <td className="text-right">
                      {inv?.invoice_only?.totalBill}
                    </td>
                    <td className="text-right">{totalDiscount || 0}</td>
                    <td className="text-right">
                      {inv?.invoice_only?.paidAmount}
                    </td>
                    <td className="text-right">{inv?.invoice_only?.due}</td>
                    <td className="text-right">{inv?.paid_amount}</td>
                  </tr>
                );
              })}
              <tr>
                <td
                  ...
                  className="text-right fw-bold"
                >
                  Total Due Collection Amount :
                </td>

                <td
                  colSpan={isSuperAdmin ? 5 : 5}
                  className="text-right fw-bold"
                >
                  {allData?.dueCollection}
                </td>
              </tr>
              <tr>
                <td
              ...
              <tr>
                ...
                <td
                  ...
                  className="text-right fw-bold"
                >
                  {allData?.paid + allData?.dueCollection - allData?.refund}
                </td>
              </tr>
            </table>
          )}
