...
const EditLabAgent = () => {
  ...
  const handleCheckBox = (e) => {
    ...
    if (checked) {
      setSpecialistArray([...specialistArray, value]);
    } else {
      setSpecialistArray(specialistArray.filter((eValue) => eValue !== value));
    }
  };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (academicArray[index].id) {
          axios
            .delete(`/remove-lab-agent-academic/${academicArray[index].id}`)
            .then((res) => {
              console.log(res, "Academic row Detele");
            });
        }
  ...
  const handleProfessionalCertificateRemoveInput = (index) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (certificateInfoArray[index].id) {
          axios
            .delete(
              `/remove-lab-agent-certificate/${certificateInfoArray[index].id}`
            )
            .then((res) => {
              console.log(res, "Academic row Detele");
            });
  ...
  const handleLicneseTrainingRemoveInput = (index) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (licneseTrainingArray[index].id) {
          axios
            .delete(
              `/remove-lab-agent-license/${licneseTrainingArray[index].id}`
            )
            .then((res) => {
              console.log(res, "training row Detele");
            });
        }
  ...
  const handleWorkExperienceRemoveInput = (index) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (workExperienceArray[index].id) {
          axios
            .delete(`/remove-lab-agent-work/${workExperienceArray[index].id}`)
            .then((res) => {
              console.log(res, "work row Detele");
            });
        }
  ...
  formData.append(
    "specialistArray",
    specialistArray.length > 0 && specialistArray.toString()
  );
  formData.append("image", informationInput.image);

  const handleSubmit = (e) => {
    e.preventDefault();
    if (informationInput.gender) {
      axios
        .post(`update-lab-agent-info/${informationInput.id}`, formData)
        .then((res) => {
          if (res.status === 200) {
            // Academic
            academicArray.map((item) => {
              if (
                ...
                academic.append("lab_agent_master_id", informationInput.id);
                ...
                if (item.id) {
                  axios
                    .post(`update-lab-agent-academic/${item.id}`, academic)
                    .then((res) => {
                  ...
                  axios
                    .post(`save-lab-agent-academic`, academic)
                    .then((res) => {
                      if (res.data.status === 200) {
                        console.log(res);
                        swal("Success", res.data.message, "success");
            ...
            certificateInfoArray.map((item) => {
              if (
                ...
                certificate.append("lab_agent_master_id", informationInput.id);
                ...
                if (item.id !== "") {
                  axios
                    .post(
                      `update-lab-agent-certificate/${item.id}`,
                      certificate
                    )
                    .then((res) => {
                      if (res.data.status === 200) {
            ...
            licneseTrainingArray.map((item) => {
              if (item.title !== "") {
                ...
                license.append("lab_agent_master_id", informationInput.id);
                ...
                if (item.id !== "") {
                  axios
                    .post(`update-lab-agent-license/${item.id}`, license)
                    .then((res) => {
                      if (res.data.status === 200) {
                        console.log(res);
                        swal("Success", res.data.message, "success");
            ...
            workExperienceArray.map((item) => {
              if (item.title !== "" && item.location && item.companyName) {
                const workExp = new FormData();
                workExp.append("lab_agent_master_id", informationInput.id);
                ...
                if (item.id !== "") {
                  axios
                    .post(`update-lab-agent-work/${item.id}`, workExp)
                    .then((res) => {
                      if (res.data.status === 200) {
                        console.log(res);
                        swal("Success", res.data.message, "success");
  ...
  const handleSubmitAcademic = (e) => {
    e.preventDefault();
    academicArray.map((item) => {
      if (item.degreeName !== "" && item.countryName && item.institutionName) {
        const academic = new FormData();
        academic.append("lab_agent_master_id", informationInput.id);
        academic.append("degreeName", item.degreeName);
        academic.append("countryName", item.countryName);
  ...
  const handleSubmitCertificate = (e) => {
    ...
    certificateInfoArray.map((item) => {
      if (item.degreeName !== "" && item.countryName && item.institutionName) {
        ...
        if (item.id !== "") {
          axios
            .post(`update-lab-agent-certificate/${item.id}`, certificate)
            .then((res) => {
              if (res.data.status === 200) {
                console.log(res);
                swal("Success", res.data.message, "success");
  ...
  useEffect(() => {
    ...
    axios.get(`edit-lab-agent/${params.id}`).then(async (res) => {
      setInformationInput(res.data.agent);
      if (res.data.academic.length > 0) {
        setAcademicArray(res.data.academic);
      }
      if (res.data.certificate.length > 0) {
        setCertificateArray(res.data.certificate);
      }
      if (res.data.training.length > 0) {
        setLicneseTrainingArray(res.data.training);
      }
      if (res.data.work.length > 0) {
        setWorkExperienceArray(res.data.work);
      }
    });
  }, [params.id]);
  return (
                ...
                <form onSubmit={handleSubmit} className="custom-card px-1">
                  ...
                  <Row>
                                ...
                                <select
                                  ...
                                  selected={
                                    orgBranch?.find(
                                      (branch) =>
                                        branch?.id ===
                                        informationInput?.saas_branch_id
                                    )?.name
                                  }
                                  onChange={(e) => {
                                    setInformationInput({
                        ...
                        <div className="row mb-2">
                          ...
                          <div className="col-3">
                            {imageUrl ? (
                              ...
                              <div className="docImage">
                                <img
                                  src={`${global.img_url}/labAgent/images/${informationInput.image}`}
                                  className="doctorImageUrlPreview"
                                  alt="preview"
                                />
                                <i
                    ...
                    <Col className="p-2 " lg={3}>
                      <p className="text-center fw-bold">Specialities</p>
