...
function RoundTreatmentProtocolName() {
  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`round-protocol-name/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
          ...
          <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mt-1 mx-2">
                {" "}
                Round Treatment Protocol Name Setup
              </h6>
...
export default RoundTreatmentProtocolName;

const emptyValues = {
  name: "",
};
const AddRoundTreatmentProtocolName = ({
  modalIsOpenForAdd,
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForAdd(false)}>
        <NewModal.Title>Add Protocol Name</NewModal.Title>
      </NewModal.Header>
...
const EditRoundTreatmentProtocolName = ({
  data,
  modalIsOpenForEdit,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`round-protocol-name/${data.id}`, form);
      if (res.status === 200) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForEdit(false)}>
        <NewModal.Title>Update Protocol Name Info</NewModal.Title>
      </NewModal.Header>
      <form onSubmit={handleSubmit}>
        <NewModal.Body>
          <div className="form-group mb-1">
