  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`cyanosis/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
        ...
        <div className="col-9">
          <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mt-1 mx-2">Cyanosis Setup</h6>
...
const AddCyanosis = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      setForm(emptyValues);
      const res = await axios.post("cyanosis", form);
      if (res.status === 200) {
        toast.success("Cyanosis added successfully");
        onRefetch();
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForAdd(false)}>
        <NewModal.Title>Add Cyanosis</NewModal.Title>
      </NewModal.Header>
...
const EditCyanosis = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`cyanosis/${data.id}`, form);
      if (res.status === 200) {
        toast.success("Cyanosis updated successfully");
        setForm(emptyValues);
        onRefetch();
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForEdit(false)}>
        <NewModal.Title>Update Cyanosis Info</NewModal.Title>
      </NewModal.Header>
      <form onSubmit={handleSubmit}>
        <NewModal.Body>
          <div className="form-group mb-1">
