...
function RoundPathologyResultSetup() {
  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`round-pathology/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
        ...
        <div className="col-9">
          <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mt-1 mx-2">Pathology Result Parameter Setup</h6>
...
export default RoundPathologyResultSetup;

const emptyValues = {
  name: "",
};
const AddPathologyResultParameter = ({
  modalIsOpenForAdd,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      setForm(emptyValues);
      const res = await axios.post("round-pathology", form);
      if (res.status === 200) {
        toast.success("Pathology Result Parameter added successfully");
        onRefetch();
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForAdd(false)}>
        <NewModal.Title>Add Pathology Result Parameter</NewModal.Title>
      </NewModal.Header>
...
const EditPathologyResultParameter = ({
  data,
  modalIsOpenForEdit,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`round-pathology/${data.id}`, form);
      if (res.status === 200) {
        toast.success("Pathology Result Parameter updated successfully");
        setForm(emptyValues);
        onRefetch();
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForEdit(false)}>
        <NewModal.Title>Update Pathology Result Parameter Info</NewModal.Title>
      </NewModal.Header>
      <form onSubmit={handleSubmit}>
        <NewModal.Body>
          <div className="form-group mb-1">
