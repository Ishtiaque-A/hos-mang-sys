  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`round-cns/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
        ...
        <div className="col-9">
          <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mt-1 mx-2">CNS Setup</h6>
...
export default RoundCnsSetup;

const emptyValues = {
  name: "",
};
const AddCNS = ({ modalIsOpenForAdd, setModalIsOpenForAdd, onRefetch }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      setForm(emptyValues);
      const res = await axios.post("round-cns", form);
      if (res.status === 200) {
        toast.success("CNS added successfully");
        onRefetch();
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForAdd(false)}>
        <NewModal.Title>Add CNS</NewModal.Title>
      </NewModal.Header>
...
const EditCNS = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`round-cns/${data.id}`, form);
      if (res.status === 200) {
        toast.success("CNS updated successfully");
        setForm(emptyValues);
        onRefetch();
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForEdit(false)}>
        <NewModal.Title>Update CNS Info</NewModal.Title>
      </NewModal.Header>
      <form onSubmit={handleSubmit}>
        <NewModal.Body>
          <div className="form-group mb-1">
