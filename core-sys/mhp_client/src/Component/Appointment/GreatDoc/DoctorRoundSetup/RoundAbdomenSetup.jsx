...
function RoundAbdomenSetup() {
  ...
  useEffect(() => {
    axios.get("round-abdomen").then((res) => {
      setData(res.data || []);
    });
  }, [refetch]);
  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`round-abdomen/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
        ...
        <div className="col-9">
          <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mt-1 mx-2">Abdomen Setup</h6>
...
export default RoundAbdomenSetup;

const emptyValues = {
  name: "",
};
const AddAbdomen = ({ modalIsOpenForAdd, setModalIsOpenForAdd, onRefetch }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      setForm(emptyValues);
      const res = await axios.post("round-abdomen", form);
      if (res.status === 200) {
        toast.success("Abdomen added successfully");
        onRefetch();
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForAdd(false)}>
        <NewModal.Title>Add Abdomen</NewModal.Title>
      </NewModal.Header>
...
const EditAbdomen = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`round-abdomen/${data.id}`, form);
      if (res.status === 200) {
        toast.success("Abdomen updated successfully");
        setForm(emptyValues);
        onRefetch();
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForEdit(false)}>
        <NewModal.Title>Update Abdomen Info</NewModal.Title>
      </NewModal.Header>
      <form onSubmit={handleSubmit}>
        <NewModal.Body>
          <div className="form-group mb-1">
