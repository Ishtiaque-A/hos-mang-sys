...
function DrugSinceCategory() {
  ...
  useEffect(() => {
    axios.get("drug-since-category").then((res) => {
      setData(res.data || []);
    });
  }, [refetch]);
  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`drug-since-category/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
        ...
        <div className="col-9">
          <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mt-1 mx-2"> Drug Since Category Setup</h6>
...
export default DrugSinceCategory;

const emptyValues = {
  name: "",
};
const AddDrugSinceCategory = ({
  modalIsOpenForAdd,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      setForm(emptyValues);
      const res = await axios.post("drug-since-category", form);
      if (res.status === 200) {
...
const EditDrugSinceCategory = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`drug-since-category/${data.id}`, form);
      if (res.status === 200) {
