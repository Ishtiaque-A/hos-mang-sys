...
function RoundChestSetup() {
  ...
  useEffect(() => {
    axios.get("round-chest").then((res) => {
      setData(res.data || []);
    });
  }, [refetch]);
  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`round-chest/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
        ...
        <div className="col-9">
          <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mt-1 mx-2">Chest Setup</h6>
...
export default RoundChestSetup;

const emptyValues = {
  name: "",
};
const AddChest = ({ modalIsOpenForAdd, setModalIsOpenForAdd, onRefetch }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      setForm(emptyValues);
      const res = await axios.post("round-chest", form);
      if (res.status === 200) {
        toast.success("Chest added successfully");
        onRefetch();
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForAdd(false)}>
        <NewModal.Title>Add Chest</NewModal.Title>
      </NewModal.Header>
...
const EditChest = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`round-chest/${data.id}`, form);
      if (res.status === 200) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForEdit(false)}>
        <NewModal.Title>Update Chest Info</NewModal.Title>
      </NewModal.Header>
      <form onSubmit={handleSubmit}>
        <NewModal.Body>
          <div className="form-group mb-1">
