...
function RoundTreatmentProtocolCycle() {
  ...
  useEffect(() => {
    axios.get("round-protocol-cycle").then((res) => {
      setData(res.data || []);
    });
  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`round-protocol-cycle/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
        ...
        <div className="col-9">
          <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mt-1 mx-2"> Round Treatment Protocol Cycle</h6>
...
export default RoundTreatmentProtocolCycle;
...
const AddTask = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      setForm(emptyValues);
      const res = await axios.post("round-protocol-cycle", form);
      if (res.status === 200) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForAdd(false)}>
        <NewModal.Title>Add Data</NewModal.Title>
      </NewModal.Header>
        ...
        <NewModal.Body>
          <div className="form-group mb-1">
            ...
            <select
              ...
              required
              onChange={(e) =>
                setForm({ ...form, protocol_id: e.target.value })
              }
...
const EditTask = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`round-protocol-cycle/${data.id}`, form);
      if (res.status === 200) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForEdit(false)}>
        <NewModal.Title>Update Data </NewModal.Title>
      </NewModal.Header>
      <form onSubmit={handleSubmit}>
        <NewModal.Body>
          <div className="form-group mb-1">
