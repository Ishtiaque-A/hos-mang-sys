...
const EditAllRateList = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`/edit-rate-list-all/${params.id}`).then(res => {
            if (res.data.status === 200) {
    ...
    const submitRateList = (e) => {
        ...
        if (rateList.name && rateList.code && rateList.categoryId) {
            axios.post(`update-rate-list-all/${params.id}`, rateListData)
                .then(res => {
                    if (res.status === 200) {
                        console.log(res)
                                ...
                                <div className="col-6">
                                    ...
                                    <div className=" mt-1">
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input mt-1" checked={rateList.preBooking ==="Yes" && true} onChange={(e) => setRateList({ ...rateList, preBooking: e.target.value })} type="radio" name="inlineRadioOptions" id="inlineRadio1" value="Yes" />
                                            <label class="" for="inlineRadio1">Yes</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input mt-1" checked={rateList.preBooking === "No" && true} onChange={(e) => setRateList({ ...rateList, preBooking: e.target.value })} type="radio" name="inlineRadioOptions" id="inlineRadio1" value="No" />
                                            <label class="" for="inlineRadio1">No</label>
                                        </div>
                                    </div>
                                    <div className="">
                                        <label htmlFor="diagnosis_procedure_code" className="col-form-label-sm"> Details </label>
                                        <textarea value={rateList.details} class="form-control" onChange={(e) => setRateList({ ...rateList, details: e.target.value })} placeholder="Enter Details Here.." id="floatingTextarea2" style={{ height: "80px" }}></textarea>
                                    </div>
                                </div>
...
export default EditAllRateList;