...
function NurseTaskCategory() {
  ...
  useEffect(() => {
    axios.get("nurse-task-category").then((res) => {
      setData(res.data || []);
    });
  }, [refetch]);
  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`nurse-task-category/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
        ...
        <div className="col-9">
          <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mt-1 mx-2"> Nurse's Task Category Setup</h6>
...
export default NurseTaskCategory;
...
const AddTaskCategory = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      setForm(emptyValues);
      const res = await axios.post("nurse-task-category", form);
      if (res.status === 200) {
...
const EditTaskCategory = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`nurse-task-category/${data.id}`, form);
      if (res.status === 200) {
