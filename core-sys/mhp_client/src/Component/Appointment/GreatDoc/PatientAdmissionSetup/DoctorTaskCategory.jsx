...
import PatientAdmissionSetupSidebar from "./PatientAdmissionSetupSidebar";

function DoctorTaskCategory() {
  ...
  useEffect(() => {
    axios.get("doctor-task-category").then((res) => {
      setData(res.data || []);
    });
  }, [refetch]);
  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`doctor-task-category/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
  ...
  return (
    <div className="mt-2 ms-2">
      <div className="row">
        <div className="col-3">
          <PatientAdmissionSetupSidebar />
        </div>
        <div className="col-9">
          <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mt-1 mx-2">Doctor's Task Category Setup</h6>
...
export default DoctorTaskCategory;
...
const AddTaskCategory = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      setForm(emptyValues);
      const res = await axios.post("doctor-task-category", form);
      if (res.status === 200) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForAdd(false)}>
        <NewModal.Title>Add Task Category</NewModal.Title>
      </NewModal.Header>
...
const EditTaskCategory = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`doctor-task-category/${data.id}`, form);
      if (res.status === 200) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForEdit(false)}>
        <NewModal.Title>Update Task Category Info</NewModal.Title>
      </NewModal.Header>
      <form onSubmit={handleSubmit}>
        <NewModal.Body>
          <div className="form-group mb-1">
