  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`nurse-task/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
...
const AddTask = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      setForm(emptyValues);
      const res = await axios.post("nurse-task", form);
      if (res.status === 200) {
...
const EditTask = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`nurse-task/${data.id}`, form);
      if (res.status === 200) {
