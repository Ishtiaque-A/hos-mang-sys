...
const TaskNurseModal = ({ isOpen, onClose, setAdmission, admission }) => {
  ...
  useEffect(() => {
    if (admission?.id) {
      setLoading(true);
      axios.get("nurse-tasklist").then((res) => {
        setData(res?.data || []);
      });
      axios.get(`nurse-saved-tasks/${admission?.id}`).then((res) => {
  ...
  const handleChange = (e, item) => {
    ...
    if (checked) {
      ...
      if (existInSaved) {
        axios.put(`nurse-task-save/${existInSaved?.id}`, { status: 0 });
        setUpdated(!updated);
      }
  ...
  const handleSubmit = (e) => {
    ...
    axios.post(`nurse-task-save`, postData).then((res) => {
    ...
    <>
      <NewModal size="lg" isOpen={isOpen} onClose={handleCloseModal}>
        <NewModal.Header isShowCloseIcon={false} onClose={handleCloseModal}>
          <NewModal.Title>Nurse's Task </NewModal.Title>
          <div>
            {showFullScreen ? (
              <img
