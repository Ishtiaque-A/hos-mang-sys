...
export default function EditGreatLabBranchRequisition({ isOpen, setIsOpen, refetch, setRefetch, stockItems, setStockItems, requisitionNo, productInfo, setProductInfo }) {
    ...
    const handleSave = (status) => {
        ...
        axios.put(`great-lab-branch-requisition/${productInfo?.id}`, data)
            .then((res) => {
                if (res.status === 200) {
                    Swal.fire({
                        position: 'center',
    ...
    const total = stockItems.reduce((total, item) => total + (parseFloat(item?.mrp || item?.price || 0) * parseFloat(item?.quantity || 0)), 0)
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/great-lab-branch-requisition-details-delete/${id}`).then((res) => {
            ...
            <form onSubmit={handleSave}>
                ...
                <Modal.Footer>
                    <div className="mb-2 rx-one-button-group">
                        <button type='button' onClick={() => handleSave("Pending")} disabled={loading || total < 1} className="btn float-end">Update</button>
                        <button type='button' onClick={() => handleSave("Approved")} disabled={loading || total < 1} className="btn float-end me-2">Approve</button>
                    </div>
                </Modal.Footer>
            </form>
        </Modal >
