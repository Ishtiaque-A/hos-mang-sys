...
export default function BranchStockOut({ id }) {
    ...
    useEffect(() => {
        setLoading(true)
        axios.post(`great-lab-branch-stock-out/${id}`, filter).then((res) => {
    ...
    const handleFilter = () => {
        axios.post(`great-lab-booth-stock-out/${id}`, filter)
            .then((res) => {
                setData(res?.data?.stock || [])
            })
    }
    ...
    return (
        <>
            <button onClick={() => setIsOpen(true)} className='btn me-2'>Stock Out</button>
            <Modal size='md' isOpen={isOpen} onClose={closeModal}>
                <Modal.Header onClose={closeModal} >
                    <Modal.Title>Stock Out Report</Modal.Title>
                </Modal.Header>
                <Modal.Body styles={{ height: "450px" }} >
                    <div className='shadow-sm p-2 mb-3 bg-body lab-points-plan rounded mt-1'>
                        ...
                        >
                            <div className="col-5">
                                <h6 className='mt-1 mx-2'>Stock Out Report</h6>
                            </div>
                            <div className='col-7'>
                                <div className="row">
                        ...
                        <div ref={componentRef} className='plan-report'>
                            <div className="print-report-header">
                                ...
                                <div className="d-flex justify-content-between">
                                    <h6 className=''>Stock Out Statement</h6>
                            ...
                            <table >
                                ...
                                {data?.map((item, index) => {
                                    let totalPoint = parseFloat(item?.price) * parseFloat(item?.quantity)
                                    grandTotal += totalPoint
                                    return (
                                        <>
                                            <tr key={index}>
                                                <td>{item?.invoice?.requisition_no}</td>
                                                <td>{moment(item?.created_at).format('DD-MM-YYYY')}</td>
                                                <td className=''>{item?.name}</td>
                                                <td className='text-right'>{item?.price}</td>
                                                <td className='text-right'>{item?.quantity}</td>
                                                {/* <td></td> */}
                                                <td className='text-right'>{totalPoint}</td>
                                            </tr>
                                        </>
                ...
                {/* <Modal.Footer>
                    <div className="mb-2 rx-one-button-group">
                        <button type='button' className="btn float-end">Save</button>
                    </div>
                </Modal.Footer> */}
            </Modal >
        </>
