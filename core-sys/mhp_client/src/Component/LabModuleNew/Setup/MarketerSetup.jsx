  ...
  const handleDelete = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`marketer/${item.id}`).then((res) => {
          if (res.status === 200) {
            setRefetch(!refetch);
  ...
  return (
    <div className="shadow-sm p-2 mb-3 bg-body rounded mt-1">
      <div className="d-flex justify-content-between align-items-center">
        <h6 className="mt-1 mx-2">Marketer Setup</h6>
...
const AddMarketer = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      setForm(emptyValues);
      const res = await axios.post("marketer-add", form);
      if (res.status === 201) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForAdd(false)}>
        <NewModal.Title>Add Marketer</NewModal.Title>
      </NewModal.Header>
...
const EditMarketer = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      e.preventDefault();
      const res = await axios.put(`marketer-update`, form);
      if (res.status === 200) {
  ...
  return (
    ...
    >
      <NewModal.Header onClose={() => setModalIsOpenForEdit(false)}>
        <NewModal.Title>Update Marketer Info</NewModal.Title>
      </NewModal.Header>
      <form onSubmit={handleSubmit}>
        <NewModal.Body>
          <div className="form-group mb-1">
