...
function ExamHistoryExtension() {
    ...
    useEffect(() => {
        axios.get(`/history-extension`).then(res => {
            if (res.status === 200) {
                console.log("history_extension", res.data);
    ...
    const columns = [
        ...
        {
            title: "SL", field: `extension_code`, render: (row) => <div>{row.tableData.id + 1}</div>
        ...
        // {
        //     title: "Exam", field: `examination_code`, render: (row) => <div>{row.examination.examination_name}</div>
        //     // title: "Exam", field: `extension_code`, render: (row) => <div>{row.tableData.id + 1}</div>
