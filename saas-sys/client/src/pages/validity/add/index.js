...
import { validityCreateApiCall } from "../../../common/apiCall/api";
...
const Page = (props) => {
  ...
  const handleSubmit = (data) => {
    ...
    validityCreateApiCall(data)
      .then((res) => {
        if (res.code === 200) {
          setResponseMessage(VALIDITY.ADD_SUCCESS_MESSAGE);
  ...
  return (
    <>
      <Box sx={{ backgroundColor: "white", mx: "10px", my: "20px", borderRadius: "30px" }}>
        <Head>
          <title>{VALIDITY.ADD_TITLE}</title>
        </Head>
        <Box
          component="main"
          sx={{
