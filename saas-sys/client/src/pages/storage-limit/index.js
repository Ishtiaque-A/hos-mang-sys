...
import { StorageLimitTable } from "../../sections/storage-limit/storage-limit-table";
...
const Page = (props) => {
  ...
  useEffect(() => {
    fetchStorageLimitAPIGet().then((response)=>{
    ...
    fetchUserOrganizationDetailsAPIGet().then((response)=>{
      if(response?.code == 200) {
  ...
  useEffect(() => {
    ...
    if (props.userOrganization?.user?.organization_id){
      let userCountLeft = parseInt(props.userOrganization?.subscriptionPlan?.user_limit??0)-props.userOrganization?.user_count;
        ...
        <Container maxWidth="xl">
          <Stack spacing={3}>
            <Stack direction="row" justifyContent="space-between" spacing={4}>
              <Stack spacing={1}>
                <Typography variant="h5" sx={{marginLeft:"10px"}}>{STORAGE.TITLE}</Typography>
              </Stack>
              <div>
                {
                  (!props.userOrganization?.user?.organization_id || userLeft > 0 ) &&
                    <Link href={ADMIN.STORAGE_ADD_URL} style={{ textDecoration: 'none' }}>
                      <Button
                        startIcon={
                ...
                <Typography  inline variant="body1" align="right" color="tomato">
                  {props.userOrganization?.user?.organization_id && userLeft > 0  && userLeft + ' User left'}
                  {props.userOrganization?.user?.organization_id && userLeft <= 0  &&  'User limit exceed, If you want to create user contact admin'}
