...
import { ValiditySearch } from "./validity-search";
...
export const ValidityTable = (props) => {
  const {
    count = 0,
    items = [],
    onPageChange = () => {},
      ...
      <Grid container spacing={2}>
        <Grid item xs={3} md={3}>
          <div style={{ width: "100%", padding: "10px" }}>
            <ValiditySearch onSearch={onSearch} />
          </div>
          ...
          <Table>
            <TableHead>
              <StyledTableRow style={{ backgroundColor: "#f2f2f2" ,borderBottom:"1px solid #F1F1F1"}}>
                ...
                <StyledTableCell>{VALIDITY.NAME}</StyledTableCell>
                <StyledTableCell>{VALIDITY.DAYS}</StyledTableCell>
                <StyledTableCell>{VALIDITY.ACTION}</StyledTableCell>
              </StyledTableRow>
            </TableHead>
            <TableBody>
              {items.map((validity, index) => {
                const isSelected = isRowSelected(validity.id);
                const actualIndex = index + page * rowsPerPage + 1;
                return (
                  <TableRow hover key={validity.id} >
                    ...
                    <TableCell padding="checkbox">
                      <Checkbox
                        ...
                        sx={{ color: "#9DA4AE" }}
                        //checked={isSelected}
                        //onChange={(event) => handleSelectOne(event, validity.id)}
                      />
                    </TableCell>
                    
                    <TableCell sx={{ color: "#9DA4AE",textDecoration:"none" }}>
                    <Link style={{ textDecoration:"none",color: "#9DA4AE" }} href={`${ADMIN.STORAGE_EDIT_URL}/${validity.id}`}>
                      {actualIndex}
                      </Link>
                      </TableCell>
                    ...
                    <TableCell>
                      ...
                      >
                        <MenuItem onClick={handleMenuClose}>
                          <Link
                            href={`validity/edit/${activeItem?.id}`}
