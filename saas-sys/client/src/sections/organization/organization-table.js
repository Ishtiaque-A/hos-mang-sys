...
import { USER, ORGANIZATION } from "../../common/constantData/language";
...
export const OrganizationTable = (props) => {
  ...
  const handleSelectAll = (event) => {
    if (event.target.checked) {
      const selectedIds = data?.data?.map((organization) => organization.id);
  ...
  const handleSelectOne = (event, id) => {
    if (event.target.checked) {
      props.setSelectedRows((prevSelectedRows) => [...props.selectedRows, id]);
    } else {
      ...
      <Grid container spacing={2}>
        <Grid item xs={2} md={3}>
          <div style={{ width: "100%", padding: "10px" }}>
            <OrganizationSearch onSearch={onSearch} />
          </div>
        </Grid>
          ...
          <Table>
            <TableHead>
              ...
              >
                <StyledTableCell padding="checkbox">
                  <Checkbox
                    ...
                    indeterminate={
                      ...
                      props.selectedRows.length < data?.data?.length
                    }
                ...
                <StyledTableCell>{ORGANIZATION.STATUS}</StyledTableCell>
                <StyledTableCell>{ORGANIZATION.ACTION}</StyledTableCell>
              </StyledTableRow>
            </TableHead>
            <TableBody>
              {data?.data?.map((organization, index) => {
                console.log(organization.status, "organization>>>>>>>>>>>>>");
                const isSelected = isRowSelected(organization.id);
                  ...
                  <TableRow hover key={organization.id}>
                    <TableCell padding="checkbox">
                      <Checkbox
                        ...
                        sx={{ color: "#9DA4AE" }}
                        checked={isSelected}
                        onChange={(event) => handleSelectOne(event, organization.id)}
                      />
                    </TableCell>

                    <TableCell sx={{ color: "#9DA4AE", textDecoration: "none" }}>
                      <Link
                        style={{ textDecoration: "none", color: "#9DA4AE" }}
                        href={`${ADMIN.ORGANIZATION_EDIT_URL}/${organization.id}`}
                      >
                        {actualIndex}
                      </Link>
                    </TableCell>
                    ...
                    <TableCell>
                      ...
                      >
                        <MenuItem onClick={handleMenuClose}>
                          <Link
                            href={`${ADMIN.ORGANIZATION_LIST}/${activeItem?.id}`}
                            style={{ textDecoration: "none", color: "#9DA4AE", fontSize: "14px" }}
                          >
