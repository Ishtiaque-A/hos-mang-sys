...
import { REFOUND, ORGANIZATION_PLAN } from "../../common/constantData/language";
        ...
        <Table>
            <TableHead>
              ...
              >
                ...
                <StyledTableCell>{REFOUND.NAME}</StyledTableCell>
                <StyledTableCell>{REFOUND.SUBSCRIPTION_PLAN}</StyledTableCell>
                <StyledTableCell>{REFOUND.AMOUNT}</StyledTableCell>
                <StyledTableCell>{REFOUND.STATUS}</StyledTableCell>
                <StyledTableCell>{REFOUND.ACTION}</StyledTableCell>
              </StyledTableRow>
            </TableHead>
            <TableBody>
              {data?.data?.map((table_data, index) => {
                ...
                ? format(new Date(table_data.purchase.subscription_detail.start_date), "MMMM dd, yyyy")
                : "N/A";
                const endDate = table_data?.purchase?.subscription_detail?.end_date
                ? format(new Date(table_data.purchase.subscription_detail.end_date), "MMMM dd, yyyy")
                : "N/A";
                return (
                  <TableRow hover key={table_data.id}>
                    ...
                    <TableCell>
                      <Stack alignItems="center" direction="row" spacing={2}>
                        <Typography variant="caption">{table_data?.cancel_request?.purchase?.organization?.name}</Typography>
                      </Stack>
                    </TableCell>
                    <TableCell>
                      <Typography variant="caption">{table_data?.cancel_request?.purchase?.subscription_plan?.name}</Typography>
                    </TableCell>
                    <TableCell>
                      <Typography variant="caption" sx={{ color: "#9DA4AE" }}>
                       {table_data?.amount? table_data?.amount :0}{" "}{props.currency}

                      </Typography>
                    </TableCell>
                    <TableCell>
                      <Typography variant="caption" sx={{ color: "#9DA4AE" }}>
                        {table_data?.status === "0" ?(<>Rejected</>):table_data?.status === "1" ?(<>Initiate</>):table_data?.status === "2" ?(<>In progress</>):(<>Closed</>)}
                      </Typography>
                    </TableCell>
                    <TableCell>
                      <IconButton
                      ...
                      >
                        <MenuItem onClick={handleMenuClose}>
                          <Link
                            href={`refund-list/${activeItem?.id}`}
                            style={{ textDecoration: "none", color: "#9DA4AE", fontSize: "14px" }}
                          >
